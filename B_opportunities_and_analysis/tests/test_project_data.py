...
class SharedDataTests(unittest.TestCase):
    ...
    def test_layout_migration_reads_old_and_stages_new_paths(self):
        self.item['shared_path'] = self.item['path']
        self.item['path'] = 'C_topic_match/data/test.csv'
        (self.root/'data-manifest.json').write_text(json.dumps(self.manifest))
        data.configure(self.source, root=self.root)
        data.ensure_data('q3', self.root)
        migrated = self.source.parent/'migrated'
        data.stage(migrated, root=self.root)
        self.assertEqual((migrated/self.item['path']).read_bytes(), self.payload)
        self.assertFalse((migrated/self.item['shared_path']).exists())
        (self.root/self.item['path']).unlink()
        data.configure(migrated, root=self.root)
        data.ensure_data('q3', self.root)
        self.assertEqual((self.root/self.item['path']).read_bytes(), self.payload)

    def test_differing_new_layout_file_does_not_fall_back_to_old(self):
        self.item['shared_path'] = self.item['path']
        self.item['path'] = 'C_topic_match/data/test.csv'
        (self.root/'data-manifest.json').write_text(json.dumps(self.manifest))
        target = self.source/self.item['path']
        target.parent.mkdir(parents=True)
        target.write_bytes(b'wrong version')
